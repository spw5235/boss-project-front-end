...
/***/ function(module, exports) {
	...
	var config = {
	  apiOrigins: {
	    ...
	    production: 'https://counselor-notepad.herokuapp.com'
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).then(function (response) {
	    ...
	    return store.user;
	  }).done(ui.signInSuccess).catch(ui.signInFailure);
	};

	var onSignUp = function onSignUp(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signUp(data).done(ui.signUpSuccess).catch(ui.signUpFailure);
	};

	var onSignOut = function onSignOut(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.signOut(data).done(ui.signOutSuccess).fail(ui.signOutFailure);
	};
	...
	var onDashButtons = function onDashButtons() {
	  $(".alert").hide();
	...
	var onCreateNewRecordBtn = function onCreateNewRecordBtn() {
	  onDashButtons();
	...
	var onShowRecordsBtn = function onShowRecordsBtn() {
	  onDashButtons();
	...
	var onGetTrackers = function onGetTrackers(event) {
	  event.preventDefault();
	  $("#create-record-btn").hide();
	  api.getTrackers().then(function (response) {
	    var tableId = $('<table id="table-gen-id" class="table-striped"></table>');
	    var tableBody = $("<tbody></tbody>");
	    var tableRowDesc = $('<tr class="tg-tr-description"><th class="tg-td-description tg-id-hide">Student ID</td><th class="tg-td-description">First Name</td><th class="tg-td-description">Last Name</td><th class="tg-td-description">Grade</td><th class="tg-td-comment tg-id-hide">Comments</td><th class="tg-td-description">View</td><th class="tg-td-description">Update</td><th class="tg-td-description">Delete</td></tr>');
	    tableBody.append(tableRowDesc);
	    var newGenRow = void 0;
	    store.numOfStudents = response.trackers.length;
	    for (var i = 0; i < store.numOfStudents; i++) {
	      var studObject = response.trackers[i];
	      var studObjectId = studObject.id;
	      var studObjectFirstName = studObject.first_name;
	      var studObjectLastName = studObject.last_name;
	      var studObjectGrade = studObject.grade;
	      var studObjectComment = studObject.comments;
	      var textclass = studObjectId.toString();
	    ...
	    var h1Title = $('<h1 class="h1-title" id="h1-dashboard-table">Counselor Dashboard:</h1>');
	    tableId.append(tableBody);
	    $(".table-generated").append(h1Title);
	    $(".table-generated").append(tableId);
	  }).done(ui.getTrackerSuccess).fail(ui.getTrackerFailure);
	  onShowRecordsBtn();
	};
	...
	var onCreateTracker = function onCreateTracker(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.createTracker(data).done(ui.createTrackerSuccess).fail(ui.createTrackerFailure);
	};

	var onDeleteTracker = function onDeleteTracker(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.deleteTracker(data).done(ui.deleteTrackerSuccess).fail(ui.deleteTrackerFailure);
	};

	var onUpdateTracker = function onUpdateTracker(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.updateTracker(data).done(ui.updateTrackerSuccess).fail(ui.updateTrackerFailure);
	};

	var onClickSignUpButton = function onClickSignUpButton() {
	...
	var onClickSignInButton = function onClickSignInButton() {
	  $("#sign-out-success").hide();
	  $("#sign-in").toggle();
	  $("#sign-up").hide();
	  $(".alert").hide();
	...
	var onClickChangePasswordBtn = function onClickChangePasswordBtn() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getTrackers = function getTrackers() {
	  return $.ajax({
	...
	var showTracker = function showTracker() {
	  return $.ajax({
	...
	var createTracker = function createTracker(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/trackers/',
	    method: 'POST',
	    headers: {
	...
	var deleteTracker = function deleteTracker() {
	  return $.ajax({
	...
	var updateTracker = function updateTracker(data) {
	  return $.ajax({
	...
	var updateTrackerinShow = function updateTrackerinShow(data) {
	  return $.ajax({
	...
	module.exports = {
	  ...
	  updateTrackerinShow: updateTrackerinShow
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getTrackerFailure = function getTrackerFailure(data) {
	  currentdata = data;
	};
	...
	var updateButtonInShow = function updateButtonInShow() {
	  $("#update-record-btn").on("click", function () {
	    store.buttonUpdateVal = store.showId;
	    var fullNameArr = $(".td-gen-name-show").text().split(" ");
	    var firstNameUpdate = fullNameArr[0].trim();
	    var lastNameUpdate = fullNameArr[1].trim();

	    var gradeArr = $(".td-gen-grade-show").text().split(": ");
	    var gradeUpdate = gradeArr[1].trim();
	...
	var showTrackerSuccess = function showTrackerSuccess(data) {
	  currentdata = data;
	  $("#show-records-btn").show();
	  $("#update-form-error").hide();
	  updateButtonInShow();
	...
	var showTrackerFailure = function showTrackerFailure(data) {
	  currentdata = data;
	};

	var createTrackerSuccess = function createTrackerSuccess(data) {
	  currentdata = data;
	  var form = document.getElementById("new-tracker-form");
	  form.reset();
	  $("#new-tracker-form").hide();
	  $(".warning").hide();
	...
	var createTrackerFailure = function createTrackerFailure(data) {
	  currentdata = data;
	  $("#new-form-error").show();
	};

	var deleteTrackerSuccess = function deleteTrackerSuccess() {
	  $(".warning").hide();
	  $("#create-record-btn").show();
	};
	...
	var updateTrackerSuccess = function updateTrackerSuccess(data) {
	  currentdata = data;
	  $("#update-tracker-form").hide();
	  $("#show-records-btn").show();
	  var form = document.getElementById("update-tracker-form");
	  form.reset();
	  $(".alert").hide();
	...
	var updateTrackerFailure = function updateTrackerFailure(data) {
	  $(".alert").hide();
	  $("#update-form-error").show();
	  currentdata = data;
	};

	var showRow = function showRow() {
	  var insertH1 = $('<h1 class="h1-title h1-show-table-generated" id="h1-show-table">Student Note:</h1>');
	  $(".view-button").on("click", function () {
	    $("#create-record-btn").hide();
	    store.buttonShowVal = parseInt($(this).attr("id"));
	    api.showTracker().then(function (response) {
	      ...
	      var showTableHtml = logic.createShowTable(store.showFn, store.showLn, store.showGrade, store.showComments);
	      $(".show-table-generated-container").append(insertH1);
	      $(".show-table-generated-container").append(showTableHtml);
	    }).done(showTrackerSuccess).fail(showTrackerFailure);
	    $("#h1-dashboard-table").remove();
	    $("#table-gen-id").remove();
	    $(".h1-title").show();
	  });
	...
	var deleteRow = function deleteRow() {
	  $(".delete-button").on("click", function () {
	    ...
	    api.deleteTracker(store.buttonDeleteVal).done(deleteTrackerSuccess).fail(deleteTrackerFailure);
	    $(this).parent().parent().hide();
	    $("#delete-form-success").show().delay(1500).slideUp();
	  });
	};

	var getTrackerSuccess = function getTrackerSuccess(data) {
	  $("#create-record-btn").show();
	  deleteRow();
	  updateRow();
	  showRow();
	...
	module.exports = {
	  ...
	  getTrackerSuccess: getTrackerSuccess,
	  getTrackerFailure: getTrackerFailure,
	  showTrackerSuccess: showTrackerSuccess,
	  showTrackerFailure: showTrackerFailure,
	  updateTrackerSuccess: updateTrackerSuccess,
	  updateTrackerFailure: updateTrackerFailure,
	  ...
	  showRow: showRow,
	  updateButtonInShow: updateButtonInShow
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var createRowHTML = function createRowHTML(tdIDtxt, TdFntxt, tdLntxt, tdGradetxt, tdCommentTxt) {

	  var tableTrGenerated = $('<tr class="tg-tr-gen"></tr>');

	  var tdID = $('<td class="tg-td-id tg-id-hide">id</td>');
	  tdID.text(tdIDtxt);

	  var tdFn = $('<td class="tg-td-fn">fn</td>');
	  tdFn.text(TdFntxt);

	  var tdLn = $('<td class="tg-td-ln">ln</td>');
	  tdLn.text(tdLntxt);

	  var tdGrade = $('<td class="tg-td-grade"></td>');
	  tdGrade.text(tdGradetxt);

	  var tdComment = $('<td class="tg-td-comment tg-id-hide"></td>');
	  tdComment.text(tdCommentTxt);

	  var tdViewBtn = $('<button type="button" class="view-button btn btn-secondary">View</button>');
	  var tdUpdateBtn = $('<button type="button" class="update-button btn btn-secondary">Update</button>');
	  var tdDeleteBtn = $('<button type="button" class="delete-button btn btn-secondary">Delete</button>');

	  var buttonIdClassTextView = tdIDtxt + "viewid";
	  var buttonIdClassTextDelete = tdIDtxt + "deleteid";
	  var buttonIdClassTextUpdate = tdIDtxt + "updateid";
	  ...
	  var tdDetailsView = $('<td class="tg-td-details-view"></td>');
	  tdDetailsView.append(tdViewBtn);

	  var tdDetailsUpdate = $('<td class="tg-td-details-update"></td>');
	  tdDetailsUpdate.append(tdUpdateBtn);

	  var tdDetailsDelete = $('<td class="tg-td-details-delete"></td>');
	  tdDetailsDelete.append(tdDeleteBtn);

	  var tableGenAppend = void 0;
	...
	var createShowTable = function createShowTable(TdFntxt, tdLntxt, tdGradetxt, tdCommentTxt) {
	  var tableId = $('<table id="table-gen-show" class="table-striped"></table>');
	  var tableBody = $("<tbody></tbody>");

	  var tableTrGeneratedName = $('<tr class="tr-gen-name-show"></tr>');
	  var tableTrGeneratedGrade = $('<tr class="tr-gen-grade-show"></tr>');
	  var tableTrGeneratedComments = $('<tr class="tr-gen-comments-show"></tr>');
	  var tableTrGeneratedUpdateBtn = $('<tr class="tr-gen-update-btn"></tr>');

	  var tdName = $('<td class="td-gen-name-show"></td>');
	  var tdGrade = $('<td class="td-gen-grade-show"></td>');
	  var tdComments = $('<td class="td-gen-comments-show"></td>');

	  // Update Button part of show table
	  var showUpdateBtn = $('<button class="show-update-btn btn btn-secondary" id="update-record-btn">Update Note</button>');
	  var tdUpdateBtn = $('<td class="td-gen-update-show"></td>');
	  var updateBtnAppend = $(tdUpdateBtn).append(showUpdateBtn);

	  var fullName = TdFntxt + " " + tdLntxt;
	  tdName.text(fullName);

	  var gradeTxt = "Grade: " + tdGradetxt;
	...
	module.exports = {
	  ...
	  createShowTable: createShowTable
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
